...
public sealed class BrowserStorage(IJSRuntime js)
{
    ...
    {
        try { await js.InvokeVoidAsync("localStorage.setItem", key, JsonSerializer.Serialize(value, JsonOptions)); }
        catch (JSException) { /* The tracker remains usable when browser storage is unavailable. */ }
    }
}
